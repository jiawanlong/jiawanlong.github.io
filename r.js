    ...
    **/
    ...
    font-size: 2rem;
    text-align: center;">
               航天宏图拖欠本人薪资，经仲裁委调解逾期数月后仍旧不予支付，请行业从业者重新评估与航天宏图的合作关系！（本人正常离职，非裁员，非被迫，纯工资，不是奖金，不是绩效）
            </div>
    ...
    function fireWork() {
            ...
            class FireworkShell {
                constructor() {
                    ...
                    this.color = `hsl(${this.hue}, 100%, ${Math.random() * 20 + 50}%)`;
                    this.trail = []; 
                    this.exploded = false; 
                }
                ...
                explode() {
                    ...
                    for (let i = 0; i < particleCount; i++) {
                        fireworksParticles.push(new FireworkParticle(this.x, this.y));
                    }
                }
                ...
                draw() {
                    ...
                    this.trail.forEach((p, index) => {
                        ctx.save();
                        ctx.globalAlpha = p.alpha * (index / this.trail.length);
                        ctx.beginPath();
            ...
            // 2. 烟花炸开粒子类（核心修改：延长停留时间）
            class FireworkParticle {
                constructor(x, y) {
                    this.x = x;
                    this.y = y;
                    this.speed = Math.random() * 3 + 0.5; // 降低初始速度，飘得更慢
                    this.angle = Math.random() * Math.PI * 2;
                    const randomHue = Math.random() * 360;
                    this.color = `hsl(${randomHue}, 100%, ${Math.random() * 20 + 50}%)`;
                    
                    // 核心修改1：大幅延长粒子生命周期（从30-80→50-130帧）
                    this.life = Math.random() * 80 + 50;
                    // 核心修改2：降低重力，粒子下落更慢
                    this.gravity = 0.01;
                    this.drag = 0.985; // 轻微增加阻力，速度衰减更慢
                ...
                update() {
                    ...
                    this.life--;
                    // 核心修改3：适配生命周期调整透明度衰减（慢衰减，更久不透明）
                    this.alpha = this.life / 130;
                    // 保证最后阶段不会突然消失，缓慢透明
            ...
            function drawNewYearText() {
                ...
                const chars = [
                    { text: '新', x: centerX - charSpacing * 1.5, hueBase: 350 },
                    { text: '年', x: centerX - charSpacing * 0.5, hueBase: 10 },
                    { text: '快', x: centerX + charSpacing * 0.5, hueBase: 30 },
                    { text: '乐', x: centerX + charSpacing * 1.5, hueBase: 50 }
                ];
        
                chars.forEach((char, index) => {
                    const rotate = Math.sin(time * 0.01 + index) * 5 * Math.PI / 180;
                    const yOffset = Math.sin(time * 1.5 + index) * 10;
                    const scale = 1 + Math.sin(time * 0.008 + index) * 0.2;
                    const hue = char.hueBase + Math.sin(time * 0.5) * 15;
        
                    ctx.save();
                    ctx.translate(char.x, centerY + yOffset);
                    ctx.rotate(rotate);
                    ctx.scale(scale, scale);
                    ctx.translate(-char.x, -(centerY + yOffset));
        
                    const gradient = ctx.createLinearGradient(
                        char.x - fontSize/3, centerY - fontSize/3,
                        char.x + fontSize/3, centerY + fontSize/3
                    );
                    gradient.addColorStop(0, `hsl(${hue}, 100%, 70%)`);
                    gradient.addColorStop(1, `hsl(${hue + 20}, 100%, 55%)`);
        
                    ctx.shadowBlur = 18;
                    ctx.shadowColor = `hsl(${hue}, 100%, 65%)`;
                    ctx.strokeStyle = `hsl(${hue + 10}, 100%, 85%)`;
                    ctx.lineWidth = 2;
                    ...
                    ctx.fillText(char.text, char.x, centerY + yOffset);
                    ctx.strokeText(char.text, char.x, centerY + yOffset);
            ...
            const MAX_SHELLS = 8; 
            const MAX_PARTICLES = 1500; // 轻微提高粒子上限，适配更长生命周期
            const startTime = Date.now();
            const duration = 6000; // 总时长延长到6秒，匹配烟花久留效果
        
            // 生成发射烟花
            function createFireworkShell() {
                if (fireworksShells.length >= MAX_SHELLS) return;
                fireworksShells.push(new FireworkShell());
            }
            ...
            function animate() {
                ...
                ctx.fillStyle = 'rgba(0, 0, 0, 0.03)'; // 更浅的拖影，适配久留效果
                ctx.fillRect(0, 0, canvas.width, canvas.height);
        
                if (elapsed < duration && Math.random() < 0.12) { // 轻微降低发射频率，避免粒子堆积
                    createFireworkShell();
                }
                ...
                // 更新+绘制炸开的粒子
                if (fireworksParticles.length <= MAX_PARTICLES) {
                    ...
                    fireworksParticles = fireworksParticles.slice(-MAX_PARTICLES / 2);
                }
        
                drawNewYearText();
        
                if (elapsed < duration || fireworksShells.length > 0 || fireworksParticles.length > 0) {
