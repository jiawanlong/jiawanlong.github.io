    ...
    **/
    ...
    function createCesiumModal() {
        ...
        let donationCode = document.createElement('div');
        ...
        // 第三行：捐赠说明
        let donationInfoSection = document.createElement('div');
        donationInfoSection.className = 'donation-info-section';
        donationInfoSection.textContent = '您捐赠的每一笔都会原封不动以您的名义捐赠到小英定向助学';

        // GIS 预览图
        let gisPreviewSection = document.createElement('div');
        gisPreviewSection.className = 'gis-preview-section';

        let gisPreview = document.createElement('div');
        gisPreview.className = 'donation-code gis-preview';

        let gisImage = document.createElement('img');
        gisImage.src = 'https://jiawanlong.github.io/gis.jpg';
        gisImage.alt = 'GIS 示例展示';

        gisPreview.appendChild(gisImage);
        gisPreviewSection.appendChild(gisPreview);
        ...
        rightSection.appendChild(gisPreviewSection);
    ...
    // 关闭弹窗函数
    function closeModal() {
        let modal = document.getElementById('cesium-modal');
        let overlay = document.getElementById('cesium-modal-overlay');
    ...
    `;
        ...
        // 警告声明段落
        let warningIntro = document.createElement('p');
    ...
    `;
        // 行动号召段落
        let callToAction = document.createElement('p');
    ...
    `;
        ...
        // 身份声明部分
        let identitySection = document.createElement('div');
    ...
    `;
        ...
        // 事实陈述与法律定性合并部分
        let factsLegalSection = document.createElement('div');
    ...
    `;
        ...
        // 对公司的正告部分
        let warningToCompany = document.createElement('div');
    ...
    `;
        ...
        // 对投资者和合作者的警告部分
        let warningToInvestors = document.createElement('div');
    ...
    console.log('我支持乌克兰');
    (function () {
        let navbarTitle = document.querySelector('.navbar-brand h1');
        if (!navbarTitle) return;
        Object.assign(navbarTitle.style, {
            background: 'linear-gradient(to bottom, #0057B7 50%, #FFD701 50%)',
            backgroundClip: 'text',
